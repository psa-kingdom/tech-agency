	...
	useEffect(() => {
		...
		for (let ix = 0; ix < AMOUNTX; ix++) {
			for (let iy = 0; iy < AMOUNTY; iy++) {
				...
				// Fade out the dots towards the end (iy = 0 is 100% transparent) and reduce overall opacity/intensity
				const maxOpacity = 0.35;
				const opacityFactor = iy / (AMOUNTY - 1);
				const intensity = opacityFactor * maxOpacity;
		...
		const animate = () => {
			...
			for (let ix = 0; ix < AMOUNTX; ix++) {
				for (let iy = 0; iy < AMOUNTY; iy++) {
					...
					// Animate Y position with sine waves (slower and flatter/subtler)
					positions[index + 1] =
